...
Vector* vector_alloc(vec_size_t alloc, vec_size_t size)
{
  Vector* v_data = (Vector*)
      malloc(sizeof(Vector) + alloc * size);
  v_data->alloc = alloc;
  return v_data;
}

Vector* vector_get_data(vector vec) { return &((Vector*)vec)[-1]; }
...
Vector* vector_realloc(Vector* v_data, vec_type_t type_size)
{
  vec_size_t new_alloc = (v_data->alloc == 0) ? 1 : v_data->alloc * 2;
  Vector* new_v_data = (Vector*)
      realloc(v_data, sizeof(Vector) + new_alloc * type_size);
  new_v_data->alloc = new_alloc;
  return new_v_data;
}

bool vector_has_space(Vector* v_data)
{
...
{
  Vector* v_data = vector_get_data(*vec_addr);
...
{
  Vector* vec_data = vector_get_data(vec);
  size_t alloc_size = sizeof(Vector) + vec_data->length * type_size;
  Vector* v = (Vector*)malloc(alloc_size);
